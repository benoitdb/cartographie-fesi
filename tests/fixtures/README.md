...
| Fichier | Origine |
|---|---|
| `data_2014-2020.json` | 1 582 opérations, même méthode d'échantillonnage appliquée au fichier Synergie 2014-2020 (issue #83). Le volume vient des 1 241 opérations interrégionales, toutes retenues par la règle « toutes les interrégionales » — dimensionnée pour les 13 de 2021-2027, elle a changé d'échelle avec l'issue #77 qui y route les 5 programmes de massif — **une fixture par période** : les deux fichiers n'ont ni les mêmes colonnes ni les mêmes fonds, et l'espace 2014-2020 ne se rendrait pas sur un échantillon de l'autre période. Champ tronqué : `Résumé de l'opération`. Pas de clé `nb_objectifs_strategiques` dans `metadata` : cette période n'a pas de dimension thématique. |
...
| `data_2014-2020_normandie.json` | 65 opérations (issue #95) : fichier régional hors-Synergie, lu directement par la page 2014-2020 pour ce périmètre (Normandie est absente de `aggregates.by_region` de Synergie — issue #68). Pas d'échantillonnage par région (un seul périmètre par fichier) : des tranches ciblées couvrent chaque fonds, dont les ~26 opérations à fonds vide de la période 2021-2023 (5 retenues). |
| `data_2014-2020_nouvelle_aquitaine.json` | 40 opérations (issue #95) : même principe, pour le fichier Nouvelle-Aquitaine (25 opérations seulement dans Synergie, sur ~4 000 réelles). Programmes nommés par code CCI dans ce fichier (`Colonne à masquer lors de la diffusion`) — traduits par `utils.periodes.appliquer_libelles_programmes`, pas par cette fixture. |
